...
uint8_t blueFontBG = BLUE;
uint8_t redFontColor = RED;
...
void ancs_notifications(ancs_notification_t* notif) {
  if ((notif->category == ANCS_CATEGORY_INCOMING_CALL) |
      ...
      (notif->category == ANCS_CATEGORY_ENTERTAINMENT) |
      (notif->category == ANCS_CATEGORY_EMAIL)) {
...
void viewNotifications(uint8_t button) {
  if (!button) {
    ...
    display.drawRect(0,menuTextY[3],96,14,TSRectangleFilled,TS_8b_Blue);
    ...
    if (amtNotifications) {
      ...
      display.fontColor(defaultFontColor, blueFontBG);
...
static const char PROGMEM mainMenuStrings0[] = "   Datum/Zeit";
static const char PROGMEM mainMenuStrings1[] = "   Auto Off";
static const char PROGMEM mainMenuStrings2[] = "   Helligkeit";
...
static const char PROGMEM dateTimeMenuStrings0[] = "     Jahr";
static const char PROGMEM dateTimeMenuStrings1[] = "     Monat";
static const char PROGMEM dateTimeMenuStrings2[] = "     Tag";
static const char PROGMEM dateTimeMenuStrings3[] = "     Stunde";
static const char PROGMEM dateTimeMenuStrings4[] = "     Minute";
...
void updateMainDisplay() {
  ...
  if (currentDisplayState == displayStateHome) {
    ...
    if (rewriteMenu || lastAmtNotificationsShown != amtNotifications) {
      ...
      if (!amtNotifications) {
        display.print(F("  Keine Nachricht "));
      } else {
        display.fontColor(redFontColor, defaultFontBG);
        display.print(F("    "));
        display.print(amtNotifications);
        display.print(F(" Nachricht "));
...
void updateTimeDisplay() {
  ...
  display.fontColor(defaultFontColor, defaultFontBG);
  if (rewriteTime || lastHourDisplayed != hr) {
    display.setFont(font22pt);
    lastHourDisplayed = hr;
    displayX = 22;
